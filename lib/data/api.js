...
function getRouteLocation(database, departureCRS, arrivalCRS) {
  // Get fetch working.
  ...
  .then(function(responseData) {
    ...
    for (i=0; i<responseData.length; i++) {
      durations.push(responseData[i].duration);
    }
    return responseData[lowestDuration(durations)].route_parts;
  }) // Find the lowest duration journey.
  .then(function(data) {
    var newData = {};
    for (i=0; i<data.length; i++) {
      if (responseData[i].mode === 'train') {
        newData[data[i].departure_time] = data[i].coordinates;
      }
    }
    database.insert();
    }).catch((error) => console.error(error));
