...
*/
  ...
  .then(function(data) {
    console.log(data);
    if ( data.error ) { throw "Cannot reach server" }
  ...
  console.log(data);
  console.log(lowestDuration(durationList));
  console.log(data[lowestDuration(durationList)]);
  return data[lowestDuration(durationList)].route_parts;

  }).then(function(routeArray) {
    var stationString = `${departure}/${arrival}`
        dataToWrite = {}
        modeIndex = 0
        dataToWrite[stationString] = {};

    for ( n = 0; n < routeArray.length; n++ ) {
      console.log(routeArray[n]);
      if ( routeArray[n].mode === 'train' | routeArray[n].mode === 'tube' ) {
        var arr = routeArray[n];
        ...
        dataToWrite[stationString][modeIndex] = {
          ...
          duration: minConvert(arr.duration)
        }
        modeIndex++;
      }
    }
    console.log(arr);
    if ( arr != null ) { console.log('Returning'); console.log(dataToWrite); return dataToWrite;
    } else if ( service != 'tfl' ) { throw "No routes found under southeast service";
    } else { throw "No routes found under tfl service"; }
