/*
  A functioning API devolving the TransportAPI web-API into node-usable functions.
...
// TODO: Change this to grab from a stored folder rather than a local folder.
  ...
  switch(action) {
    ...
    case 'time':
      if (endTime != null) {
        ...
        url = path.join(baseURL, 'train', 'station', stationCRS,
                        'live.json?app_id=' + apiObj.id + '&app_key=' + apiObj.key
                        + '&darwin=true&to_offset=PT00:02:00&train_status=passenger');
      }
    return new URL(url);
    break;
  }
