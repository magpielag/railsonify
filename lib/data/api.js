...
async function getRouteLocation(database, departureCRS=null, arrivalCRS=null, railService='southeast') {
...
*/
  ...
  url = buildAPIAddress('location', departureCRS, arrivalCRS, service=railService);
  await request(url)
  ...
  .then(function(data) {
    console.log(`Serverside route data`);
    console.log(data);
    ...
    for (i=0; i<routeParts.length; i++) { // Iter through journey 'legs' and omit all those not using a train.
      console.log(`Modes: ${routeParts[i].mode}`);
    ...
    console.log('ToWrite obj');
    console.log(toWrite);

    if (Object.values(toWrite).length == 0) {
      console.log('Rerunning!')
      if (railService = 'southeast') {
        throw ('Southeast not TFL');
      } else {
        throw ('No train routes on TFL.')
      }
    }
    // Catch case where no rail route is found ('usually because of an incorrect service tag').
    database.insert(toWrite, function(err, doc) {});
  }).catch((error) => {
    if (error === "Southeast not TFL") {
      return getRouteLocation(database, departureCRS, arrivalCRS, service='tfl')
    } else {
      console.error(error);
      return;
    }
  ...
  switch(action) {
    ...
    case 'loc':
      ...
      url = path.join(baseURL, 'public', 'journey', 'from', 'lonlat:' +
                      ...
                      api.id + '&app_key=' + api.key + '&service=' + service);
