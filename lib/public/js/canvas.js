...
const bounds = {
    x: { min: -8.642594970884725, max: 1.7624989748001116 },
...
function getRouteArray(departureSrc, arrivalSrc) {
  ...
  $.ajax({
         ...
         {
            console.log(data);
            var data = scaleBoundaries(height, width, data[0].route);
            draw(data);
            delete data;
        },
         error: function (data) {
...
$('#hello').click(function(e) { // Change this to run from the scheduler.
  console.log('Click');
  getRouteArray('NWT', 'KGX');
});
