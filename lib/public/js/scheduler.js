...
//async function(dep, arr, op) { return getRouteArray(dep, arr, op); }
var timeDifference = function(timeValue) {
  var now = new Date();
  console.log('DEBUG TIME: ', timeValue);
...
class Scheduler {
  ...
  constructor(timetable) {
    ...
    self.setCalls = function() {
      ...
      var calls = [];
      console.log('DEBUG DEPARTURES:', departures);
      ...
      departures.forEach(function(dataObj) {
          ...
          console.log(departTime != null, timeoutTime < 150000, timeoutTime > -150);
          if ( departTime != null && timeoutTime < 150000 && timeoutTime > 0 ) {
...
async function startScheduler(sendData, url) {
  console.log('sending: ', sendData, url);
      $.ajax({
             type: "POST",
             url: url,
...
function startWithRandom(button) {
  ...
  $.ajax({
         ...
         {
            console.log('Recieved: ', stationCodes);
