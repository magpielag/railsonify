...
class MapSynth {
  ...
  play() {
    ...
    console.log('DS: ', routeDuration / routeArray.length);
    ...
    var playInterval = setInterval(async function() {
      if ( routeIndex >= routeArray.length ) {
        ...
        voices.forEach(function(voice) {
          console.log('DS: ', voice.frequency.value);
          if ( voiceIdx != 0 ) {
            if ( voiceIdx == 1 ) {
              var freqMod = 1.25;
          ...
          voice.frequency.linearRampToValueAtTime(f, context.now() + rampTime);
          voice.frequency.setValueAtTime(f, context.now() + rampTime + 0.01);
        });
        routeIndex++;
      }
