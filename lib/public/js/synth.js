...
function resumeContext(button) {
  ...
  Tone.context.resume();
  var ms = new MinimalSynth({route: [[149, 500], [593, 500]], duration: 10, late:true});
...
class MinimalSynth {
  constructor(data) {
    ...
    self.play = function(coord) {
      console.log(coord, _bounds.x.min);
    ...
    self.start = function() {
      // Reverb set by distance; Pan set by x; Frequency set by y; Two notes.
      let coordinates = [self.data.route[0], self.data.route[self.data.route.length-1]];
      var panValue;
      var reverbValue;
      var chord;
      ...
      coordinates.forEach(function(coord) {
        if ( coord == coordinates[1] ) {
          // Find absolute difference between x and y and add together.
          let coordValue = Math.abs(coord[0] - coordinates[0][0]) + Math.abs(coord[1] - coordinates[0][1]);
          reverbValue = Math.round(self.scale.lin(coordValue, 0, (_bounds.x.max - _bounds.x.min) + (_bounds.y.max - _bounds.y.min), 0.0, 1.2) * 10) / 10;
          self.reverb.roomSize.setValueAtTime(reverbValue, self.synth.voices[2].envelope.release);
          setTimeout(async function() { self.play(coord); }, (self.synth.voices[2].envelope.release+2.5) * 1000);
        } else {
          self.play(coord);
        }
      });
    ...
    self.connect();
    //self.reverb.roomSize.value = 0.5;
    self.start();
  }
}
