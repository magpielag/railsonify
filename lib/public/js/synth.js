  ...
  */
  constructor(data) {
    ...
    */
    ...
    self.change = {
      pan: async function(value, ramp) {
        self.autoFilter.depth.value = value;
        self.dynamics.pan.pan.linearRampToValueAtTime(value, ramp);
        self.dynamics.pan.pan.setValueAtTime(value, ramp);
      },

      frequency: async function(value, ramp) {
        self.autoFilter.baseFrequency = value[0]/2;
        var _j = 0;
        self.synths.forEach(async function(s) {
          s.frequency.linearRampToValueAtTime(value[i], ramp);
          i++;
//setTimeout(function() { s.frequency.value = value; }, ramp*1000);
        });
      }
    };
    ...
    self.play = async function(frequency, pan) {
      ...
      */
      ...
      if ( self.on == false ) { // If the synth is not currently triggered (down).
        ...
        self.change.frequency(frequency, rampTime);
        /*
        for ( var synthN = 0; synthN < self.synths.length; synthN++ ) {
          console.log('BOOM');
          // Set synths to new frequency value over a ramp time (same as interval).
          if ( frequency != null ) {
            console.log('RAMP IT BROOOOOOOOOOOOOOOOOOOOOOO', frequency);
...
*/
    ...
    this.start = self.start = async function() {
      ...
      */
      ...
      let harmonicIdx = [ root, ( root + noteMod ) % rootKeys.length,
       ...
       */
       ...
       var playInterval = setInterval(async function() {
         if ( playIndex == dataObject.route.length ) {
         ...
         var panValue = await self.scale.lin('x', dataObject.route[playIndex][0], -1.0, 1.0);
         var unscaledFrequency = await self.scale.log('y', dataObject.route[playIndex][1], self.scale.min, self.scale.max);
         var scaledFrequency = await self.scale.closest(unscaledFrequency, noteArray);
         if ( self.fCheck.value == 0 ) { self.fCheck.value = scaledFrequency;
           self.fCheck.b = true;
         } else if ( self.fCheck.value == scaledFrequency ) {
           self.fCheck.b = false;
         } else {
           self.fCheck.value = scaledFrequency;
           self.fCheck.b = true;
         }
         if ( self.fCheck.b ) {
           console.log('Frequency passes:', scaledFrequency);
           let chord = await makeChord(scaledFrequency, dataObject.late);
           self.play(chord, panValue);
         } else {
           self.change.pan(panValue, self.context.now() + (self.interval/1000));
         }
         /*Scale to Hz value then map to fit the closest value (if wanting a chord.)
           if ( playIndex == 0 | playIndex == dataObject.route.length-1 ) {
             ...
             self.play(scaledFrequency, panValue);
          }
        }*/
        playIndex++;
    ...
    */
    ...
    self.data = data;
    self.synths = self.getSynths(3, ['triangle', 'square', 'square']);
    self.autoFilter = new Tone.AutoFilter( {
        frequency: 9,
        type: 'sine',
        baseFrequency: 16.35,
        depth: 1,
    ...
    self.interval = self.totalTime / self.data.route.length;
    self.fCheck = {value: 0, b: false};
    for ( let s = 0; s < self.synths.length; s++ ) {
      console.log('Setting up ', self.synths[s]);
      self.synths[s].chain(self.autoFilter, self.dynamics.gain, self.dynamics.pan, Tone.Master);
    }
