...
server.post('/', async function(req, res) {
  ...
  */
  updateInterval = setInterval(async function() {
    var currentTime = new Date().getTime(); //Get the current time, might have to format away date information or convert
    // from millis string (if so, write something in time.js to do so).
    tools.db.update(timetable, currentTime);
  }, 600);
});
