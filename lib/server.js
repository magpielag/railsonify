...
server.post('/update', async function(req, res) {
  var _stationCode = req.body;
  var _currentDate = new Date();
  var _currentTime = `${_currentDate.getHours()}:${_currentDate.getMinutes()}`;
  timetable = await tools.db.update(timetable, _currentTime);
  /*timetable.find({}, function(error, documents) {
    if (!error) {
      for ( docIndex = 0; docIndex < documents.length; docIndex++ ) {
        var _docCode = Object.keys(documents[docIndex]);
        if (_docCode == _stationCode) { res.send()}
      }
    } else { throw error; }
  });*/
  tools.db.grab(res, timetable, _stationCode);
});
