 ...
 */
 ...
 database.find( {} , async function( error, documents ) {
   if ( !error ) {
     // Iterate through documents to find both each key and each document relating to that key.
     for (i=0; i<documents.length; i++){
       var docList = Object.keys(documents[i]); // Gets station codes.
       var timetableList = documents[i][docList[0]]; // Get contents of station docs.
       ...
       if ( timetableList != null ) {
         ...
         for ( d = 0; d < departures.length; d++ ) {
           ...
           if ( !compareTime(departure, time) ) {
             database.remove({_id:documents[i][docList[1]]}, { multi: true }, function(err, removed) {
      ...
      // Brute-force approach: Store the crs, delete whole db and store new values.
      await store(database, 'timetable', {station: docList[0]})
